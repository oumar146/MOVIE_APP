            ...
            {props.movie && (
                <div >
                    ...
                    <div className='button-watch-now horizontal-alignment'>
                        ...
                        <NavLink to={`/movie-page/${props.movie.id}`} className='navlink'>Regarder maintenant</NavLink>
                    </div>
                </div>
            )}
        </div >
    ...
    useEffect(() => {
        if (bannerMovie && bannerMovie.length > 0) {
            setBackgroundImageStyle({
                ...
                backgroundImage: `linear-gradient(to bottom, rgba(0, 0, 0, 0), #131834), url(https://image.tmdb.org/t/p/original/${bannerMovie[0].backdrop_path})`,
            });
        }
    }, [bannerMovie]);
